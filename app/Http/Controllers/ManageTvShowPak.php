...
class ManageTvShowPak extends Controller
{
    ...
    public function getTvShowPakList(Request $request, $id){
        ...
        $records = TvShowPak::orderBy($columnName, $columnSortOrder)
            ...
            ->when(request()->has('status') && request()->status !== null, function ($query) {
                        $query->where('tv_shows_pak.status', request()->status);
                    }) 
        ...
        foreach ($records as $record) {
            if($record->status == 1){
                $status = '<a onchange="updateStatus(\''.url('tv-show-pak/update-status',base64_encode($record->id)).'\')" href="javascript:void(0);"><label class="switch s-primary mr-2"><input type="checkbox" value="1" checked id="accountSwitch{{$record->id}}"><span class="slider round"></span></label> </a>';
            }else{
                $status = '<a onchange="updateStatus(\''.url('tv-show-pak/update-status',base64_encode($record->id)).'\')" href="javascript:void(0);"><label class="switch s-primary   mr-2"><input type="checkbox" value="0" id="accountSwitch{{$record->id}}"><span class="slider round"></span></label></a>';
            }
    ...
    public function add(Request $request)
    {
        ...
        if (!empty($request->hidden_countries) && is_array($request->hidden_countries)) {
            $tvshow->countries = implode(',', $request->hidden_countries);
        } else {
            $tvshow->countries = ''; // ya empty string '' rakho agar column string hai
        }
    ...
    public function editTvShow($id)
    {
        ...
        $this->data['movieContries'] = explode(',', $tvshow->countries);
