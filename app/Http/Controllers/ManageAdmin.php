...
class ManageAdmin extends Controller
{
        ...
        foreach ($records as $record) {
            ...
            $html = $wallet_amount.' 
                <a href="javascript:void(0)" 
                    ...
                    data-url="'.route('admin.revert-amount', $record->id).'"
            ...
            $data_arr[] = array(
                ...
                "action" => $record->id == 1 ? '<div class="action-btn">
                        ...
                        <a href="/app-domain/'.base64_encode($record->id).'" title="App Domain Content">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" 
                        ...
                        <path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06
                            a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 
                            0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 
                            1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 
                            0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06a1.65 
                            ...
                            1.65 0 0 0 1.82.33h.09a1.65 1.65 0 0 0 1-1.51V3a2 
                            2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 
                            0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06a1.65 
                            1.65 0 0 0-.33 1.82v.09a1.65 1.65 0 0 0 1.51 1H21a2 
    ...
    public function resellerLogin(Request $request, $id){
        $id = base64_decode($id);
        $data = User::find($id);
        ...
        if (!Hash::check($data->real_password, $user->password)) {
    ...
    public function revertAmount(Request $request, $id)
    {
        ...
        if ($request->amount > $admin->current_amount) {
            return response()->json([
                'message' => 'Amount exceeds admin balance'
            ], 400);
        }

        try {

            \DB::transaction(function () use ($request, $current_user, $admin) {

                // 🔻 deduct from admin
                $admin->current_amount -= $request->amount;
                $admin->save();
                ...
                SadminWallet::create([
                    ...
                    'amount_method' => 'Cash',
                    'message' => 'Amount Revert from super admin '.$admin->name.' ('.$admin->email.')',
                ]);
                ...
                AdminWallet::create([
                    ...
                    'credit_amount_by' => $current_user->id,
                    'amount_method' => 'Cash',
                    'message' => 'Amount Revert by super admin '.$current_user->name.' ('.$current_user->email.')',
                ]);
