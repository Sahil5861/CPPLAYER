...
use App\Models\RetailorAds;
use App\Models\AdminAds;

class RetailorAd extends Controller
{
    public function index()
    {
        return view('admin.retailor_ads.index');
    }

    /* Process ajax request */
    public function getRetailorAdsList(Request $request)
    {
        ...
        // Total records
        // Total records
        $totalRecords = RetailorAds::select('count(*) as allcount')->whereNull('retailor_ads.deleted_at')->where('user_id', \Auth::user()->id)->count();
        $inactiveRecords = RetailorAds::select('count(*) as allcount')->where('status','0')->whereNull('retailor_ads.deleted_at')->where('user_id', \Auth::user()->id)->count();
        $activeRecords = RetailorAds::select('count(*) as allcount')->where('status','1')->whereNull('retailor_ads.deleted_at')->where('user_id', \Auth::user()->id)->count();
        $deletedRecords = RetailorAds::select('count(*) as allcount')->whereNotNull('retailor_ads.deleted_at')->where('user_id', \Auth::user()->id)->count();


        $totalRecordswithFilter = RetailorAds::select('count(*) as allcount')
        ->where(function ($query) use ($searchValue) {
            $query->whereNull('retailor_ads.deleted_at')
                ->where('retailor_ads.user_id', \Auth::user()->id)
                ->where('retailor_ads.title', 'like', '%' . $searchValue . '%');
        })
        ->count();

        // Get records, also we have included search filter as well
        $records = RetailorAds::orderBy($columnName, $columnSortOrder)
            // ->where('retailor_ads.status', '=', 1)
            ->whereNull('retailor_ads.deleted_at')
            ->where('retailor_ads.title', 'like', '%' . $searchValue . '%')
            ->where('retailor_ads.user_id', \Auth::user()->id)            
            ->select('retailor_ads.*')
            // ->leftJoin('plans', 'plans.id', '=', 'RetailorAds.RetailorAds_id')
            ->skip($start)
            ->take($rowperpage)
            ->get();
    ...
    public function create(){  
        ...
        return view('admin.retailor_ads.add', $this->data);
    }

    public function edit($id){                
        $id = base64_decode($id);
        $this->data['super_admin_ads'] = AdminAds::whereNull('deleted_at')->get();
        $this->data['ad_plan'] = RetailorAds::where('id', $id)->first();
        return view('admin.retailor_ads.add', $this->data);
    }
    ...
    public function save(Request $request){
        ...
        $admin_ad_plan = !empty($request->id) ? RetailorAds::firstwhere('id',$request->id) : new RetailorAds();                       
    ...
    public function destroy(Request $request){
        $ad_plan = RetailorAds::where('id',base64_decode($request->id))->first();
    ...
    public function updateStatus($id){
        $ad_plan = RetailorAds::find(base64_decode($id));
        if($ad_plan){
