    ...
    protected function updatePlan($data_user, $creater){
        ...
        if ($last_plan) {
            ...
            if (($current_user_amount >= $price) && $planDetails) {
                ...
                if ($plan->save()) {
                    $old_plans = UserPlanDetails::where('user_id', $data_user->id)->where('plan_id', $plan_id)->get();

                    if (count($old_plans) > 0) {
                        UserPlanDetails::where('user_id', $data_user->id)->where('plan_id', $plan_id)->delete();
                    }
