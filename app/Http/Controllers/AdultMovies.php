...
class AdultMovies extends Controller
{
    ...
    public function getAdultMoviesList(Request $request)
    {
        ...
        foreach ($records as $record) {
            if ($record->status == 1) {
                $status = '<a onchange="updateStatus(\'' . url('above-18/update-status', base64_encode($record->id)) . '\')" href="javascript:void(0);"><label class="switch s-primary mr-2"><input type="checkbox" value="1" checked id="accountSwitch{{$record->id}}"><span class="slider round"></span></label> </a>';
            } else {
                $status = '<a onchange="updateStatus(\'' . url('above-18/update-status', base64_encode($record->id)) . '\')" href="javascript:void(0);"><label class="switch s-primary   mr-2"><input type="checkbox" value="0" id="accountSwitch{{$record->id}}"><span class="slider round"></span></label></a>';
            }
    ...
    public function save(Request $request)
    {
        ...
        $movie = !empty($request->id) ? AdultMovie::firstwhere('id', $request->id) : new AdultMovie();
        ...
        if ($movie->save()) {
            ...
            $message = !empty($request->id) ? 'Movie updated successfully' : 'Movie added successfully';
    ...
    public function editChannel($id)
    {
        ...
        // print_r($this->data['channelGenre']); exit;
    ...
    {
        $last_order = AdultMovie::max('movie_order');
    ...
    public function importCsv(Request $request)
    {
        ...
        while (($row = fgetcsv($file)) !== false) {
            ...
            $movie = AdultMovie::updateOrCreate(
