...
class ManageUser extends Controller
{
    ...
    public function getUserList(Request $request)
    {
        ...
        foreach ($records as $record) {
            ...
            $last_plan_details = UserPlanDetails::where('user_id', $record->id)->where('plan_end_date','>=',now())->where('status', 1)->orderBy('created_at', 'desc')->first();            
            $last_plan = '';
            $last_plan_id = null;

            $mac_address = $record->mac_address ? $record->mac_address : '<span class="text-warning">Not Set</span>';
            $mac_address_app = $record->mac_address_app ? $record->mac_address_app : '<span class="text-warning">Not Set</span>';
            ...
            $data_arr[] = array(
                ...
                "mac_address_app" => $mac_address_app,
                ...
                "plan_expire_date" => ($last_plan_details) ? date('j M Y h:i a',strtotime($last_plan_details->plan_end_date)) : '<span class="text-warning">No Active Plan</span>'              
            );
        }
    ...
    public function add(Request $request){
        $request->validate([
            'name' => 'required',            
            'mobile' => 'required',
            'box_mac_address' => 'nullable|string|max:255',
        ]);
        ...
        if(!empty($request->id)){
            ...
            $user->email = $request->email ?? '';
            $user->mobile = $request->mobile;
            $user->box_mac_address = strtoupper(trim((string) ($request->box_mac_address ?? '')));
            $user->address = $request->address ?? '';
            $user->hf_number = $request->hf_number ?? '';
            $user->street_number = $request->street_number ?? '';
            $user->landmark = $request->landmark ?? '';
            ...
            if($user->save()){
                ...
                if($request->has('user_plan') && $request->user_plan != ''){
                    ...
                    if(($value == 1000 || $value > 1000) && $value < 10000){ //superadmin plan_id started from 1000
                        ...
                        if(\Auth::user()->role == 2){ //admin plan_id started from 10000
                            ...
                            $planDetails = RetailorPlan::find($value);
                        }        
                        $user_purchase_price +=  $planDetails->total_price;
                    }
                    ...
                    $plan->role = (
                        ...
                        (
                            (\Auth::user()->role == 3) ? 'reseller' :
                            (
                                (\Auth::user()->role == 4) ? 'retailor' :
                                'netadmin'
                            )
                        )
                    );
                ...
                if ($request->has('updatePin') || $request->has('updateAppPin') || $request->has('updateOver18Pin')) {
                    ...
                    elseif (!$last_active_plan) {
                        ...
                        elseif ($role == 4) {
                            $last_plan = UserPlanDetails::select('user_plan_details.*', 'retailor_plans.total_price as net_price')
                                    ->leftJoin('retailor_plans', 'retailor_plans.id', '=', 'user_plan_details.plan_id')
                                    ->where('user_plan_details.user_id', $user->id)->orderBy('user_plan_details.id', 'desc')->first();
                        }  
    ...
    public function editUser($id){
        ...
        // print_r(now());exit;    
        $userPlan = UserPlanDetails::where('user_plan_details.user_id',base64_decode($id))->where('plan_end_date','>=',now())->where('user_plan_details.status',1)->get();
        ...
        foreach ($userPlan as $key => $value) {
            ...
            if(($value->plan_id == 1000 || $value->plan_id > 1000) && ($value->plan_id < 10000)){
                ...
                }else if ($value->role == 'reseller'){
                    $plans[] = ResellerPlan::where('status',1)->where('id',$value->plan_id)->whereNull('deleted_at')->first();
                }
                else if ($value->role == 'retailor'){
                    $plans[] = RetailorPlan::where('status',1)->where('id',$value->plan_id)->whereNull('deleted_at')->first();
                 }
            }
        }
        // echo "<pre>";
        ...
        if(\Auth::user()->role == 2){
            ...
            $plans = RetailorPlan::where('user_id',\Auth::user()->id)->where('status',1)->whereNotIn('id',$existing_plan)->whereNull('retailor_plans.deleted_at')->get();
    ...
    public function checkWalletAdmin(Request $request){
        ...
        if(isset($request->plan_id) && $plan_id != ''){
            ...
            else if ($auth_user_role ==3){
                // reseller plan
                $Plans = ResellerPlan::where('id',$plan_id)->first();
            }
            else if($auth_user_role == 4){
                // retailor paln
                $Plans = RetailorPlan::where('id',$plan_id)->first();
            }
    ...
    public function updateUserFreezeStatus(Request $request, $id){
        $id = base64_decode($id);

        $user = User::where('id', $id)->first();

        if ($user) {
            $user->freeze_status = $user->freeze_status == 0 ? 1 : 0;

            $user->save();

            $message = $user->freeze_status == 1 ? 'User Freezed Successfully !' : 'User Unfreezed Successfully !';
