...
use App\Models\RetailorWallet;
use Illuminate\Support\Facades\Hash;

class ManageRetailers extends Controller
{
    public function index(){
        $this->data['admins'] = User::where('role',4)->get();
        return view('admin.retailor.index',$this->data);
    }

    public function getRetailorsList(Request $request){
        ...
        if($loggedUser->role == 1){
            // Total records
            $totalRecords = User::select('count(*) as allcount')->where('role',4)->whereNull('users.deleted_at')->count();
            $inactiveRecords = User::select('count(*) as allcount')->where(['role' => 4,'status'=>'0'])->whereNull('users.deleted_at')->count();
            $activeRecords = User::select('count(*) as allcount')->where(['role' => 4,'status'=>'1'])->whereNull('users.deleted_at')->count();
            $deletedRecords = User::select('count(*) as allcount')->where('role',4)->whereNotNull('users.deleted_at')->count();
            ...
            // Total records
            $totalRecords = User::select('count(*) as allcount')->where(['users.role'=>4,'users.created_by'=>$loggedUser->id])->whereNull('users.deleted_at')->count();
            $inactiveRecords = User::select('count(*) as allcount')->where(['users.role'=>4,'users.created_by'=>$loggedUser->id])->where('status','0')->whereNull('users.deleted_at')->count();
            $activeRecords = User::select('count(*) as allcount')->where(['users.role'=>4,'users.created_by'=>$loggedUser->id])->where('status','1')->whereNull('users.deleted_at')->count();
            $deletedRecords = User::select('count(*) as allcount')->where(['users.role'=>4,'users.created_by'=>$loggedUser->id])->whereNotNull('users.deleted_at')->count();

            $totalRecordswithFilter = User::select('count(*) as allcount')
            ->where('users.created_by',$loggedUser->id)->where('role',4)->whereNull('users.deleted_at')
            ->where('name', 'like', '%' . $searchValue . '%')
            ...
            // Get records, also we have included search filter as well
            $records = User::orderBy($columnName, $columnSortOrder)
            ->where('users.created_by',$loggedUser->id)->where('role',4)
            ->whereNull('users.deleted_at')
            ->where('users.name', 'like', '%' . $searchValue . '%')
            ->whereNull('users.deleted_at')
            ->orWhere(function($query) use ($searchValue,$loggedUser)
        ...
        foreach ($records as $record) {
            ...
            $wallet_amount = number_format($record->current_amount,2);

            $html = $wallet_amount.' 
                <a href="javascript:void(0)" 
                    data-amount="'.$record->current_amount.'" 
                    data-url="'.route('retailer.revert-amount', $record->id).'"
            ...
            $freeze_status = '
                <label class="switch s-danger mr-2">
                    <input type="checkbox" 
                        '.($record->freeze_status == 1 ? 'checked' : '').'
                        onclick="handleFreezeToggle(this)"
                        data-status="'.$record->freeze_status.'"
                        data-url="'.url('update-user-freeze-satus', base64_encode($record->id)).'"
                        id="freezeSwitch'.$record->id.'">
            ...
            $data_arr[] = array(
                "name" => $record->name,
                "email" => $record->email,
                "wallet_amount" => \Auth::user()->role == 3 ?  $html : $wallet_amount,
                ...
                "action" => '<div class="action-btn">
                                <a  href="edit-retailer/'.base64_encode($record->id).'"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></a>
                                <a href="javascript:;" onclick="delete_item(\''.base64_encode($record->id).'\',\'retailor\')"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></a>
                            </div>',
                "signIn" => '<a href="reseller-signin/'.base64_encode($record->id).'" data-toggle="tooltip" data-placement="top" title="Sign IN"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-log-out"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path><polyline points="16 17 21 12 16 7"></polyline><line x1="21" y1="12" x2="9" y2="12"></line></svg></a>',
            );
        }
    ...
    public function getResellerListByAdmin(Request $request){
        ...
        $totalRecordswithFilter = User::select('count(*) as allcount')->where('users.created_by',$request->admin_id)->where('role',4)->count();

        $records = User::where('users.created_by',$request->admin_id)->where('role',4)->whereNull('users.deleted_at')->get();
        $data_arr = array();
    ...
    public function add(Request $request){
        ...
        if(!empty($request->id)){
            ...
            if($user->save()){
                return back()->with('message','Retailer updated successfully');
            }else{
                return back()->with('message','Retailer not updated successfully');
            }
            ...
            if($user->save()){
                return back()->with('message','Retailer added successfully');
            }else{
                return back()->with('message','Retailer not added successfully');
            }
        }
    ...
    public function updateStatus($id){
        ...
        if($user){
            ...
            $user->save();
            echo json_encode(['message','Retailor status successfully']);
        }else{
            echo json_encode(['message','Something went wrong!!']);
        }
    }

    public function editRetailor($id){
    ...
    public function destroy(Request $request){
        ...
        if($user->save()){
            echo json_encode(['message','Retailor deleted successfully']);
        }else{
            echo json_encode(['message','Retailor not deleted successfully']);
        }
    }
    ...
    public function revertAmount(Request $request, $id)
    {
        $request->validate([
        ...
        if ($request->amount > $retailer->current_amount) {
            return response()->json([
                'message' => 'Amount exceeds retailer balance'
            ], 400);
        }

        try {

            \DB::transaction(function () use ($request, $current_user, $retailer) {

                // 🔻 deduct from retailer
                $retailer->current_amount -= $request->amount;
                $retailer->save();

                // 🔺 add to admin
                $current_user->current_amount += $request->amount;
                $current_user->save();

                // 🔹 admin wallet entry (create new record instead of update)
                ResellerWallet::create([
                    'user_id' => $current_user->id,
                    'credit_amount' => $request->amount,
                    'credit_amount_by' => $current_user->id,
                    'amount_method' => 'Cash',
                    'message' => 'Amount Revert from retailer '.$retailer->name.' ('.$retailer->email.')',
                ]);

                // 🔹 retailer wallet entry
                RetailorWallet::create([
                    'user_id' => $retailer->id,
                    'debit_amount' => $request->amount,
                    'credit_amount_by' => $current_user->id,    
                    'amount_method' => 'Cash',
                    'message' => 'Amount Revert by reseller '.$current_user->name.' ('.$current_user->email.')',
                ]);
