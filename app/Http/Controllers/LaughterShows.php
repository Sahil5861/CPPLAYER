...
class LaughterShows extends Controller
{
    ...
    public function getLaughterShowOrderList(){
        $this->data['movies'] = Laughterhow::whereNull('deleted_at')->orderBy('index', 'asc')->get();
    ...
    public function add(Request $request){
        ...
        if(!empty($request->id)){
            ...
            $laughter_show->backup_url = $request->bk_url ?? null;
            $laughter_show->genres = implode(',', $request->movie_genre);
            if (!empty($request->hidden_countries) && is_array($request->hidden_countries)) {
                $laughter_show->countries = implode(',', $request->hidden_countries);
            } else {
                $laughter_show->countries = ''; // ya empty string '' rakho agar column string hai
            }
    ...
    public function editlaughtershow($id){  
        ...
        $this->data['movieContries'] = explode(',', $laughter_show->countries);
    ...
    public function saveLaughterShowOrder(Request $request)
    {
        ...
        if (!empty($ids)) {
            foreach ($ids as $index => $id) {
                Laughterhow::where('id', $id)->update([
                    'movie_order' => $index + 1,
                    'index' => $index + 1
                ]);
            }
