...
class RetailorPlanController extends Controller
{
    public function index(){
        return view('admin.retailor_plan.index');
    }

    public function getRetailorPlanList(Request $request){
        ...
        // Total records
        // Total records
        $totalRecords = RetailorPlan::select('count(*) as allcount')->where('user_id', \Auth::user()->id)->whereNull('retailor_plans.deleted_at')->count();
        $inactiveRecords = RetailorPlan::select('count(*) as allcount')->where('user_id', \Auth::user()->id)->where('status','0')->whereNull('retailor_plans.deleted_at')->count();
        $activeRecords = RetailorPlan::select('count(*) as allcount')->where('user_id', \Auth::user()->id)->where('status','1')->whereNull('retailor_plans.deleted_at')->count();
        $deletedRecords = RetailorPlan::select('count(*) as allcount')->where('user_id', \Auth::user()->id)->whereNotNull('retailor_plans.deleted_at')->count();


        $totalRecordswithFilter = RetailorPlan::select('count(*) as allcount')->where('user_id', \Auth::user()->id)->whereNull('retailor_plans.deleted_at')
        ->orWhere('retailor_plans.title', 'like', '%' . $searchValue . '%')
        ->count();

        // Get records, also we have included search filter as well
        $records = RetailorPlan::orderBy($columnName, $columnSortOrder)
            // ->where('retailor_plans.status', '=', 1)
            ->whereNull('retailor_plans.deleted_at')
            ->where('user_id', \Auth::user()->id)
            ->where('retailor_plans.title', 'like', '%' . $searchValue . '%')
            ->orWhere(function($query) use ($searchValue)
            {
                $query->where('retailor_plans.description', 'like', '%' . $searchValue . '%')
                ->where('user_id', \Auth::user()->id)
                ->whereNull('retailor_plans.deleted_at');

            })
            ->select('retailor_plans.*')
        ...
        foreach ($records as $record) {
            if($record->status == 1){
                $status = '<a onchange="updateStatus(\''.url('retailorPlan/update-status',base64_encode($record->id)).'\')" href="javascript:void(0);"><label class="switch s-primary mr-2"><input type="checkbox" value="1" checked id="accountSwitch{{$record->id}}"><span class="slider round"></span></label> </a>';
            }else{
                $status = '<a onchange="updateStatus(\''.url('retailorPlan/update-status',base64_encode($record->id)).'\')" href="javascript:void(0);"><label class="switch s-primary   mr-2"><input type="checkbox" value="0" id="accountSwitch{{$record->id}}"><span class="slider round"></span></label></a>';
            }
            ...
            $data_arr[] = array(
                ...
                "action" => '<div class="action-btn">
                        <a  href="edit-retailorPlan/'.base64_encode($record->id).'"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></a>
                        <a href="javascript:;" onclick="delete_item(\''.base64_encode($record->id).'\',\'retailorPlan\')"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></a>
                      </div>',
            );
        }
    ...
    public function updateStatus($id){
        $plan = RetailorPlan::find(base64_decode($id));
        if($plan){
    ...
    public function addRetailorPlan(){
        $this->data = array();
        ...
        $this->data['admin_plans'] = ResellerPlan::select('reseller_plans.*')->where('reseller_plans.status',1)->where('reseller_plans.user_id',\Auth::user()->created_by)->whereNull('reseller_plans.deleted_at')->get();

        // print_r($this->data);exit;
        return view('admin.retailor_plan.add',$this->data);
    }
    ...
    public function add(Request $request){
        ...
        $plan = !empty($request->id) ? RetailorPlan::firstwhere('id',$request->id) : new RetailorPlan();
        ...
        $message = !empty($request->id) ? 'Plan updated successfully' : 'Plan added successfully';
        if($plan->save()){
    ...
    public function editRetailorPlan($id){
        ...
        $this->data['plan'] = RetailorPlan::where('id',base64_decode($id))->first();
    ...
    public function destroy(Request $request){
        $plan = RetailorPlan::where('id',base64_decode($request->id))->first();
    ...
    public function getPlanDetails(Request $request){        
        ...
        $super_admin_plan = $request->admin_plan;
        $admin_plan_id = $request->admin_plan_id;


        // print_r($admin_plan_id);exit;
        if($admin_plan_id){
            return response()->json([
                'status' => true,
                'data' => ResellerPlan::where('id',$admin_plan_id)->first()
            ]);
        }
        else{
            return response()->json([
