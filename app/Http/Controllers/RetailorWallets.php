...
class RetailorWallets extends Controller
{
    public function index(){
        return view('admin.wallet.retailor-wallet-list');
    }

    public function getRetailorWalletList(Request $request) {
        ...
        $totalRecords = RetailorWallet::select('count(*) as allcount')->where('user_id',\Auth::user()->id)->orWhere('credit_amount_by',\Auth::user()->id)->count();

        $total_amount_added = RetailorWallet::where('user_id',\Auth::user()->id)->sum('credit_amount');

        $amount_credit_to_admins = RetailorWallet::where('credit_amount_by',\Auth::user()->id)->sum('debit_amount');

        $total_credit_number = RetailorWallet::select('count(*) as total_credit_number')->where('user_id', \Auth::user()->id)->count();
        $total_debit_number = RetailorWallet::select('count(*) as total_debit_number')->where('credit_amount_by', \Auth::user()->id)->count();


        $totalRecordswithFilter = DB::select("SELECT count(*) as allcount  FROM credit_debit_retailor_amount cr LEFT JOIN clientusers cu ON IF(cr.credit_amount=0.00, cu.id=cr.user_id, cu.id=cr.credit_amount_by) WHERE cr.user_id = ".\Auth::user()->id." OR cr.credit_amount_by = ".\Auth::user()->id." AND cr.credit_amount LIKE '%".$searchValue."%' OR cr.debit_amount LIKE '%".$searchValue."%' AND cr.deleted_at IS NULL ORDER BY cr.created_at desc");

        // Get records, also we have included search filter as well
        // $records = RetailorWallet::orderBy($columnName, $columnSortOrder)
        //     ->where('credit_debit_retailor_amount.credit_amount_by',\Auth::user()->id)
        //     ->whereNull('credit_debit_retailor_amount.deleted_at')
        //     ->where('credit_debit_retailor_amount.credit_amount', 'like', '%' . $searchValue . '%')
        //     ->orWhere(function($query) use ($searchValue)
        //     {
        //         $query->where('credit_debit_retailor_amount.debit_amount', 'like', '%' . $searchValue . '%')
        //         ->whereNull('credit_debit_retailor_amount.deleted_at')
        //         ->where('credit_debit_retailor_amount.credit_amount_by',\Auth::user()->id);

        //     })
        //     ->orWhere(function ($query){
        //         $query->where('credit_debit_retailor_amounts.user_id', '=', \Auth::user()->id);
        //     })
        //     // ->orWhere('credit_debit_retailor_amounts.user_id', '=', \Auth::user()->id)
        //     ->select('credit_debit_retailor_amounts.*')->with('user')
        //     // ->leftJoin('wallets', 'wallets.id', '=', 'AdminWallet.AdminWallet_id')
        //     ->skip($start)
        //     ->take($rowperpage)
        //     ->get();

        $records = DB::select("SELECT *, cr.id as transaction_id  FROM credit_debit_retailor_amount cr LEFT JOIN users cu ON cu.id = cr.user_id WHERE cr.user_id = ".\Auth::user()->id." OR cr.credit_amount_by = ".\Auth::user()->id." AND cr.credit_amount LIKE '%".$searchValue."%' OR cr.debit_amount LIKE '%".$searchValue."%' AND cr.deleted_at IS NULL ORDER BY cr.created_at desc LIMIT ".$rowperpage." OFFSET ".$start);
    ...
    public function addRetailorWallet(){
        $this->data['users'] = User::where(['role'=>4,'created_by'=>\Auth::user()->id, 'status' => 1])->whereNull('users.deleted_at')->get();
        return view('admin.wallet.retailor-wallet',$this->data);
    }
