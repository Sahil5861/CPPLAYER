...
class ManageTvShowEpisode extends Controller
{
    ...
    public function getshowSeasonList(Request $request, $id){
        ...
        $records = (clone $query)->orderBy($columnName, $columnSortOrder)
            ...
            ->when(request()->has('status') && request()->status !== null, function ($query) {
                        $query->where('shows_episodes.status', request()->status);
                    }) 
    ...
    {
        $last_order = TvShowEpisode::max('episode_number');
        $headers = [
    ...
    public function importCsv(Request $request)
    {
        ...
        while (($row = fgetcsv($file)) !== false) {
            ...
            if ($this->checkIsExist($title, $videoUrl, $season_id)) {
                continue;
            }

            $count = TvShowEpisode::whereNull('deleted_at')
                ->where('season_id', $season_id)
                ->count();
            ...
            $episode = new TvShowEpisode();
            $episode->title = $title;
            $episode->episode_number = $data['Episode Number'];
            $episode->episoade_order = $data['Episode Number'];
            $episode->season_id = $season_id;
            $episode->video_url = $videoUrl;
            $episode->status = $status;
            $episode->streaming_type = 'youtube';
            $episode->description = $data['Description'] ?? $title;
            $episode->thumbnail = $data['Thumbnail'] ?? null;
            $episode->duration = 0;
            $episode->playlist_id = null;
            $episode->release_date = null;
            $episode->save();
