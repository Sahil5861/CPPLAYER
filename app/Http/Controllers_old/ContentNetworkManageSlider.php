...
class ContentNetworkManageSlider extends Controller
{
    public function index($id)
    {
        $id = base64_decode($id);
        return view('admin.content_slider.index', compact('id'));
    }
    ...
    public function getSliderList(Request $request)
    {
        ...
        $id = $request->id;
        $totalData = ContentSlider::whereNull('deleted_at')->where('content_network_id', $id)->count();
        ...
        {
        $sliders = ContentSlider::offset($start)
        ->whereNull('deleted_at')
        ->where('content_network_id', $id)
        ->limit($limit)
        ->orderBy($order,$dir)
        ->get();
        }
        ...
        $sliders = ContentSlider::where('id','LIKE',"%{$search}%")
        ->whereNull('deleted_at')
        ->where('content_network_id', $id)
        ...
        $totalFiltered = ContentSlider::where('id','LIKE',"%{$search}%")
        ->orWhere('title', 'LIKE',"%{$search}%")
        ->count();
        }
        ...
        if(!empty($sliders))
        {
            foreach ($sliders as $slider)
            {
                ...
                $slidersData['content_type'] = $slider->slider_for == 'movies' ? 'Movie' : 'Web Series';
                if($slider->status == 1){
                    // $slidersData['status'] = 'Active';
                    $slidersData['status'] = '<a onchange="updateStatus(\''.url('content-network-slider/update-status',base64_encode($slider->id)).'\')" href="javascript:void(0);"><label class="switch s-primary mr-2"><input type="checkbox" value="1" checked id="accountSwitch{{$slider->id}}"><span class="slider round"></span></label> </a>';
                }else{
                     // $slidersData['status'] = 'Inactive';
                    $slidersData['status'] = '<a onchange="updateStatus(\''.url('content-network-slider/update-status',base64_encode($slider->id)).'\')" href="javascript:void(0);"><label class="switch s-primary   mr-2"><input type="checkbox" value="0" id="accountSwitch{{$slider->id}}"><span class="slider round"></span></label></a>';
                }
                ...
                $slidersData['action'] = '<div class="action-btn">
                        ...
                        <a href="/edit-content-network-slider/'.base64_encode($slider->id).'"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></a>
    ...
    public function addslider($id){
        ...
        return view('admin.content_slider.add', compact('id'));

    }

    public function updateStatus($id){
        $slider = ContentSlider::find(base64_decode($id));        
    ...
    public function add(Request $request){
        ...
        if(!empty($request->id)){
            $slider = ContentSlider::firstwhere('id',$request->id);            
            $slider->banner = $request->image;
            $slider->title = $request->title;            
            $slider->slider_for = $request->slider_for;                      
            $slider->content_network_id = $request->content_network_id;            
    ...
    public function editSlider($id){
        $slider = ContentSlider::where('id',base64_decode($id))->first();
        ...
        return view('admin.content_slider.add',$this->data);
    }

    public function destroy(Request $request){
        // $slider = slider::firstwhere('id',$request->id);
        $slider = ContentSlider::where('id',base64_decode($request->id))->first();
