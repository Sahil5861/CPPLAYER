...
class ManageTvChannel extends Controller
{
        ...
        if (empty($request->input('search.value'))) {
            $tvchannels = TvChannel::whereNull('deleted_at')
                ->when(request()->has('status') && request()->status !== null, function ($query) {
                        $query->where('tv_channels.status', request()->status);
                    })
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
    ...
    public function addtvchannel()
    {
        ...
        return view('admin.tvchannel.add', compact('languages', 'content_networks'));
    }
    ...
    public function add(Request $request)
    {
        ...
        if ($tvchannel->save()) {
            TvShowContentNetwork::where('show_id',$tvchannel->id)->delete();
            DB::table('content_network_log')->where('content_id', $tvchannel->id)->where('content_type', '4')->delete();                                                                

            if ($request->has('content_networks') && !empty($request->content_networks)) {                                        
                foreach ($request->content_networks as $key => $network) {
                    $MovieNetwork = new TvShowContentNetwork();
    ...
    public function editTvChannel($id)
    {
        ...
        $this->data['current_networks'] = TvShowContentNetwork::where('show_id', base64_decode($id))->get()->pluck('network_id')->toArray();
