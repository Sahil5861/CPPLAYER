...
class SettingsController extends Controller
{
    public function index(Request $request){
        ...
        return view('admin.url_settings.index', compact('content_networks'));
    }
    ...
    public function add(){
        $content_networks = ContentNetwork::all();
        return view('admin.url_settings.add', compact('content_networks'));
    }
    ...
    public function save(Request $request){
        ...
        switch ($content_type) {
            case '1':
                $content = Movie::where('movie_url', 'LIKE', '%' . $old_url . '%')->get();
                break;
            
            case '2':
                $content = WebSeriesEpisode::where('url', 'LIKE', '%' . $old_url . '%')->get();
                break;

            case '4':
                $content = TvShowEpisode::where('video_url', 'LIKE', '%' . $old_url . '%')->get();
                break;

            case '5':
                $content = TvShowEpisodePak::where('video_url', 'LIKE', '%' . $old_url . '%')->get();
                break;

            case '6':
                $content = KidsshowsEpisode::where('url', 'LIKE', '%' . $old_url . '%')->get();
                break;

            case '7':
                $content = RelshowsEpisode::where('url', 'LIKE', '%' . $old_url . '%')->get();
                break;

            case '8':
                $content = TournamentMatches::where('video_url', 'LIKE', '%' . $old_url . '%')->get();
                break;

            case '9':
                $content = StageshowPak::where('movie_url', 'LIKE', '%' . $old_url . '%')->get();
                break;

            case '10':
                $content = Laughterhow::where('movie_url', 'LIKE', '%' . $old_url . '%')->get();
                break;
            default:
                $content = [];
