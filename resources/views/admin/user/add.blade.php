                ...
                <!-- <div class="row"> -->
                    <form id="user-form"  method="post" action="{{route('saveUser')}}" enctype="multipart/form-data" novalidate class="simple-example" >
                        ...
                        <div class="form-row">
                            ...
                            <div class="col-md-6 mb-4">
                                <label for="box_mac_address">MAG / Box MAC Address</label>
                                <input type="text" class="form-control" id="box_mac_address" name="box_mac_address" placeholder="00:1A:79:84:F7:9D" value="{{ old('box_mac_address', isset($user) ? $user->box_mac_address : '') }}">
                                <small class="form-text text-muted">This MAC address will be used for MAG / STB device mapping.</small>
                                <div class="invalid-feedback">
                                    @error('box_mac_address') {{ $message }} @enderror
                                </div>
                            </div>
                            ...
                            <div class="col-md-6 mb-4">
                                <label for="fullName">Add additional wallet Credits</label>
                                <input type="text" class="form-control" id="wallet_amount" name="wallet_amount" placeholder="Add wallet Credits" value="{{old('wallet_amount')}}@if(isset($user)){{$user->wallet_amount}}@endif">
                                <div class="invalid-feedback">
                                    @error('wallet_amount') {{ $message }} @enderror
                                </div>
                                {{-- @if(isset($user))<p style="color: green;">Current Wallet {{env('CREDITS_TEXT')}}: {{$user->current_amount}}</p>@endif --}}
                                @if(isset($user))<p style="color: green;">Current Wallet Credits: {{$user->current_amount}}</p>@endif
                            </div>
                            @if(isset($user))
                            <div class="col-md-6 mb-4">
                                <label for="fullName">Choose Plan</label>
                                <select onchange="checkWallet(this.value)" name="user_plan" id="user_plan" class="form-control select">
                                    ...
                                    foreach($packages as $key => $plan){
                                        if(isset($user) && $user->user_plan_id == $plan['id']){
                                            ...
                                            echo '<option value="'.$plan['id'].'">'.$plan['title'].' /'.round($plan['total_price'],2).' Credits</option>';
                                        }
