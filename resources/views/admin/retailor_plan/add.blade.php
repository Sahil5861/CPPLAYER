    ...
    <div class="row layout-top-spacing">
        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
            <div class="widget-content widget-content-area br-6">
                ...
                <!-- <div class="row"> -->
                    <form id="user-form"  method="post" action="{{route('saveRetailorPlan')}}" enctype="multipart/form-data" novalidate class="simple-example" >
                        @csrf
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="title" id="admin_plan" class="form-control select" required onchange="admin_plan1()">
                                    ...
                                    @foreach ($admin_plans as $i => $plans)
                                        <option value="{{$plans->title}}" {{ (isset($plan) && $plan->title == $plans->title) ? 'selected' : '' }} data-title="{{$plans->title}}"  data-id="{{ $plans->id }}"  data-counter="{{$i}}">{{$plans->title}}</option>
                                    @endforeach
                                </select>
                            </div>
...
<script>
    ...
    function admin_plan1(){
        ...
        if(admin_plan.length){
            $.ajax({
              url: "{{ url('get-reseller-plan-details') }}",
              method: 'post',
              data: {
                 'admin_plan': admin_plan,
              ...
              success: function(result){
                 if(result.status){
                    ...
                    var total_price = data.total_price + profit_price;
