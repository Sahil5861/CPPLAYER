...
export class ChannelsListPage implements OnInit {
  ...
  /** 🔹 Header */
  networkName: string = 'TV Channels';
  ...
  getNetworks() {
    ...
    this.apiService
      .post('getNetworks', { data_for: 'tvshows' })
      .subscribe((res: any) => {
  ...
  /** 🔹 Unified navigation (same as content-network & tvshows) */
  goTo(data: any){
    // localStorage.setItem('channelName',data.name)
    // this.router.navigate(['/tv-channels/shows-list', data.id]);
    localStorage.setItem("selectedTvData", JSON.stringify(data));
    localStorage.setItem('categoryId',data.id)
    this.router.navigate(['/tv-channels/episodes-list', data.id]);
