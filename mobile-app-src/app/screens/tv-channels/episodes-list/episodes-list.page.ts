...
export class EpisodesListPage implements OnInit {
  ...
  ngOnInit() {
    ...
    const data = localStorage.getItem('selectedTvData');
    if (data) {
  ...
  loadSeasons() {
    ...
    this.apiService.get(`getShowSeasons/${this.seasonsId}`).subscribe(
      (res: any) => {
  ...
  loadEpisodes(seasonId: number) {
    ...
    this.apiService.get(`getShowSeasonsEpisodes/${seasonId}`).subscribe(
      (res: any) => {
