...
@ViewChild('video') myVideo?: ElementRef;
  ...
  ngOnInit() {
    ...
    const data = localStorage.getItem('selectedKidsData');
    if (data) {
  ...
  loadSeasons() {
    ...
    this.apiService.get(`getKidsShowSeasons/${this.seasonsId}`).subscribe(
      (res: any) => {
  ...
  loadEpisodes(seasonId: number) {
    ...
    this.apiService.get(`getKidsShowSeasonsEpisodes/${seasonId}`).subscribe(
      (res: any) => {
  ...
  playVideo(obj: any) {
    ...
    if (['youtube', 'youtubelive'].includes((obj.source || '').toString().toLowerCase())) {
        this.router.navigate(['/player'], {
          queryParams: { url: obj.url },
        });
      } else {
        console.log('playvideowithurl');
        this.apiService.playVideoWithUrl(obj.url);
      }
  }
