...
export class ChannelsListPage implements OnInit {
  ...
  getNetworks() {
    ...
    this.apiService
      .post('getNetworks', { data_for: 'kidchannels' })
      .subscribe((res: any) => {
  ...
  selectNetwork(network: any) {
    ...
    this.networkName = network.name || 'Kids Channels';
  ...
  /** 🔹 Unified navigation (same as content-network & kidchannels) */
  goTo(data: any){
    // localStorage.setItem('channelName',data.name)
    // this.router.navigate(['/kids/shows-list', data.id]);
